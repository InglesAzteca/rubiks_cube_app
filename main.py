...
class RubiksApp(customtkinter.CTk):
    ...
    color_details = [
        ...
        {'color_name': 'default', 'color_reference': 'd',
         'main_color': '#1f6aa5', 'hover_color': '#144870'}
    ]
    ...
    def cube_rotation(self, cube_state, X_Y_Z, amount=1, prime=1):
        ...
        for number_of_rotations in range(amount):
            ...
            for r in rotation[::prime]:
                r = r[::prime]
                cube_state[r[0]], cube_state[r[1]] = cube_state[r[1]], cube_state[r[0]]  # swaps the values in the cube
    ...
    def get_dictionary_details(self, detail_dictionaries, reference_value=None,
                               return_value=None):
    ...
    def enable_toggle_buttons(self):
        """Sets the state of each toggle button to normal/clickable."""
        for check_box in self.check_box_details:
            check_box['check_box'].configure(state=tkinter.NORMAL)

    def enable_color_palette(self):
        """Sets the state of each color palette button to normal/clickable and
        sets the color to the main color."""
        for palette_button in self.color_palette_buttons.values():
            palette_button.configure(state='normal',
                                     fg_color=palette_button.fg_color[0])  # fg_color is a tuple
